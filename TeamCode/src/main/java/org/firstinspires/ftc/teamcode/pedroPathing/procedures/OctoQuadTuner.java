...
public class OctoQuadTuner extends Procedure {
  ...
  public void run() throws InterruptedException {
    ...
    Inputs.Field<OctoQuadTuner.PodType> podType =
        inputs
            .e("Odometry Pod Type", OctoQuadTuner.PodType.class)
            .withDefault(OctoQuadTuner.PodType.FOUR_BAR);
    Inputs.Field<OctoQuad.I2cRecoveryMode> recoveryMode =
        inputs
            .e("Recovery Mode", OctoQuad.I2cRecoveryMode.class)
    ...
    Inputs inputsHeadingScalar =
        inputs(
            "Custom Scalar Identification Turns",
            "Set the number of times you will turn your robot.");
    Inputs.Field<Integer> turns = inputsHeadingScalar.i("Turns").withDefault(10);
    awaitInputs(inputsHeadingScalar);
    double headingScalar =
        runOpMode(
            new OctoQuadHeadingScalar(octoquadName.get(), turns.get(), xPort.get(), yPort.get()));
    ...
    if (podType.get() == PodType.CUSTOM) {
      ...
      customPodScalar =
          runOpMode(
              new OctoQuadCustomPodScalar(
                  distance.get(), octoquadName.get(), xPort.get(), yPort.get()));
    }

    boolean forwardPodReversed =
        runOpMode(
            new OctoQuadForwardDirection(
                octoquadName.get(),
                podType.get(),
                customPodScalar,
                headingScalar,
  ...
  public OctoQuadHeadingScalar(String name, int turns, int xPodPort, int yPodPort) {
    super(
        ...
        "Determines the scalar for the custom pods of the OctoQuad localizer. \n"
            ...
            + " degrees exactly ("
            + turns
            + " times) exactly and then stop the OpMode.",
        true);
    this.name = name;
    this.turns = turns;
        ...
        new OctoQuadConfig(
            c -> {
              ...
              if (podType == OctoQuadTuner.PodType.CUSTOM) {
                ...
                c.ticksPerUnit.set(
                    podType == OctoQuadTuner.PodType.SWING_ARM
                        ? OctoQuadTuner.SWING_ARM
                        : OctoQuadTuner.FOUR_BAR);
              }
            });
    OctoQuadLocalizer localizer = new OctoQuadLocalizer(hardwareMap, config);
    localizer.setPose(new Pose(0, 0));
